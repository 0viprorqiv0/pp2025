...
class StudentMarkSystem:
    def __init__(self):
        ...
        if storage.dat_exists():
            storage.decompress()
            self.students = storage.load_students(Student)
            self.courses  = storage.load_courses(Course)
            self.marks    = storage.load_marks(Mark)
    ...
    def add_student_ui(self, stdscr):
        ...
        storage.write_students(self.students)
    ...
    def add_course_ui(self, stdscr):
        ...
        storage.write_courses(self.courses)
    ...
    def input_mark_ui(self, stdscr):
        ...
        storage.write_marks(self.marks)
    ...
    def show_marks_ui(self, stdscr):
        ...
        rows = []
        for s in self.students:
            score = next((m.value for m in self.marks
                          if m.student_id == s.id and m.course_id == cid), "N/A")
    ...
    def menu(self, stdscr):
        ...
        while True:
            stdscr.clear()
            h, w = stdscr.getmaxyx()
            stdscr.addstr(1, w//2 - 10, "USTH SYSTEM")
            ...
            if k == curses.KEY_UP:
                ...
                if cur == 0: self.add_student_ui(stdscr)
                    ...
                    method = curses_input(stdscr, "Compression (gz/none):")
                    storage.compress(method if method in ("gz", "none") else "gz")
                    break
